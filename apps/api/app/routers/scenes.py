...
async def list_nodes(chapter_id: str = Query(alias="chapterId"), db: AsyncSession = Depends(get_db)):
    return await scenes_service.list_for_chapter(db, chapter_id)


@router.post("/scene-nodes", status_code=status.HTTP_201_CREATED, response_model=SceneNodeOut)
...
async def update_node(node_id: str, body: SceneNodeUpdateInput, db: AsyncSession = Depends(get_db)):
...
async def create_choice(body: ChoiceOptionCreateInput, db: AsyncSession = Depends(get_db)):
...
async def update_choice(option_id: str, body: ChoiceOptionUpdateInput, db: AsyncSession = Depends(get_db)):
